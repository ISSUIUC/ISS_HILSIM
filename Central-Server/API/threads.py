...
class WebsocketThread(threading.Thread):
    ...
    def setup_callbacks(self, on_connect_callback: callable, on_message_callback: callable, on_disconnect_callback: callable) -> None:
    ...
    def __init__(self, websocket_port): 
        ...
        self.thread_id = "M-ws" # Main-websocket (thread_id doesn't matter, as long as it doesn't overlap)
        self.running = True
        ...
        # This line serves ./static/ws_page.html when attempting to connect to the websocket page with http.
...
class BoardThread(threading.Thread): 
    ...
    def __init__(self, thread_name: str, thread_id: str, board_id: int, communication_channel: communication_interface.CommunicationChannel, pop_back_callback: callable) -> None: 
        ...
        self.is_ready = False # True when in READY state (? TODO: check if this is true)
    ...
    def run_job(self):
        ...
        self.packet_buffer.add(self.cur_job_config) # Send the job by adding it to the packet buffer (See: packets.DataPacketBuffer)
        print(f"(comm:#{self.thread_id})", f"[run_job]", f"Job initialization command sent", flush=True)
...
class BoardManagerThread(threading.Thread):
    ...
    def create_threads(self):
        ...
        while len(self.spin_up_queue) > 0:
            ...
            thr = BoardThread(t.thread_name, t.thread_name, GLOBAL_BOARD_ID, t.communicaton_channel, self.pop_killed_job_back_to_queue)
