...
def run_hilsim(raw_csv: str, serial_port: serial.Serial, update_callback):
    ...
    print("(stream_data) Request accepted: Approximate runtime: " + str(approx_time_sec) + "s")    
    ...
    while(True):      
        ...
        if time.time()*1000 > last_time + 10:
            ...
            if time.time()*1000 < start_time + 5000:
                ...
                if(cur_line == 0):
                    packet.construct_job_status(True, "running", f"Running (Data streaming started)")
                cur_line += 1

                if(cur_line % 300 == 0):
                    update_callback(packet.construct_job_status(True, "running", f"Running ({cur_line/len(csv)*100:.2f}%) [{cur_line} processed out of {len(csv)} total]"))

                line_num, row = next(csv_list, (None, None))
                if line_num == None:
                    update_callback(packet.construct_job_status(True, "done", f"Finished data streaming"))
                    break
                data = csv_datastream.csv_line_to_protobuf(row)
                if not data:
                    update_callback(packet.construct_job_status(True, "error", f"Expected data to insert, but found none."))
                    return hilsim_return_log
                try:
                    serial_port.write(data)
                except:
                    update_callback(packet.construct_job_status(True, "error", f"Exception during serial write: " + traceback.format_exc()))
                    return hilsim_return_log
