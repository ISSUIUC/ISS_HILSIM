...
def run_setup_job(Server: Datastreamer.DatastreamerServer):
    ...
    try:
        ...
        accepted_status = pkt.JobStatus(
            pkt.JobStatus.JobState.SETUP, "ACCEPTED", "Setting up job " + str(job.job_id))
...
def handle_job_packet(packet: pkt.DataPacket):
    ...
    if (Datastreamer.instance.state.server_state == SState.JOB_SETUP or Datastreamer.instance.state.server_state ==
            SState.JOB_RUNNING or Datastreamer.instance.state.server_state == SState.JOB_ERROR):
    ...
    Datastreamer.instance.current_job_data = pkt.JobData(
        job['job_id'], pkt.JobData.GitPullType(
            job['pull_type']), job['pull_target'], pkt.JobData.JobType(
            job['job_type']), job['job_author_id'], pkt.JobData.JobPriority(
                job['job_priority']), job['job_timestep'])
    Datastreamer.instance.current_job = avionics.HilsimRun(
        Datastreamer.instance,
        avionics.av_instance,
        raw_csv,
        Datastreamer.instance.current_job_data)

    Datastreamer.instance.state.try_transition(
        Datastreamer.ServerStateController.ServerState.JOB_SETUP)
