    ...
    for packet in packets:
        ...
        match packet.packet_type:
            ...
            case pkt.DataPacketType.ACKNOWLEDGE:
                ...
                if (Server.server_comm_channel is not None):
                    Server.packet_buffer.add(pkt.CL_INVALID(packet))
            case pkt.DataPacketType.REASSIGN:
                # For REASSIGN, we check if the command is valid, reassign if
                # yes, INVALID if no.
            ...
            case pkt.DataPacketType.JOB:
                ...
                if (Server.job_active):
                    Server.packet_buffer.add(
                        pkt.CL_BUSY(Server.current_job_data))
                else:
...
def add_always_events(statemachine: Datastreamer.ServerStateController):
    ...
    statemachine.add_always_event(
        Datastreamer.ServerStateController.ServerState.ANY,
        handle_server_packets)
